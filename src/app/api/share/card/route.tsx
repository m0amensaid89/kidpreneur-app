...
export async function GET(req: NextRequest) {
  ...
  const xpLabel = locale === 'ar' ? 'نقطة' : 'XP'
  const earned = locale === 'ar' ? 'حقق إنجازاً رائعاً' : 'just unlocked'
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 24, color: '#B45309', textAlign: 'center', marginBottom: 20, opacity: 0.8 }}>{earned}</div>
        ...
        <div style={{ background: '#FDE68A', borderRadius: 50, padding: '10px 28px', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 48 }}>
          <span style={{ fontSize: 24 }}>⚡</span>
          <span style={{ fontSize: 28, fontWeight: 900, color: '#92400E' }}>+{xp} {xpLabel}</span>
        </div>
        <div style={{ fontSize: 20, color: '#D97706', fontWeight: 700 }}>kidpreneur.i-gamify.net</div>
      </div>
    ),
    { width: w, height: h }
  )
